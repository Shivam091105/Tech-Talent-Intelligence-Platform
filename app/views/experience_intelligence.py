"""
Experience Intelligence — career-level analysis.
"""
...
def render():
    page_header("Experience Intelligence", "Demand and compensation across career levels")
    ...
    try:
        tab1, tab2, tab3 = st.tabs(["Distribution", "Skills by Level", "Work Mode by Level"])

        with tab1:
            col1, col2 = st.columns(2)
            with col1:
                section_header("Job Demand by Experience Level")
                dist_df = experience_analytics.get_experience_distribution(country=country, source_dataset=source)
            ...
            with col2:
                ...
                sal_df = experience_analytics.get_salary_by_experience(country=country, source_dataset=source)
                if not sal_df.empty:
        ...
        with tab2:
            section_header("Top Skills per Experience Level",
                           "How skill requirements change across career stages")
            skills_df = experience_analytics.get_skills_by_experience(
                limit_skills=8, country=country, source_dataset=source
            )
            if not skills_df.empty:
                fig = grouped_bar(skills_df, x="level_name", y="job_count", color="skill_name", barmode="group")
                st.plotly_chart(fig, use_container_width=True)
            else:
                empty_state("Not enough data")

        with tab3:
            section_header("Work Mode by Experience Level",
                           "Remote work availability across career stages")
            wm_df = experience_analytics.get_work_mode_by_experience(country=country, source_dataset=source)
            if not wm_df.empty:
                fig = grouped_bar(wm_df, x="level_name", y="job_count", color="work_mode", barmode="stack")
                st.plotly_chart(fig, use_container_width=True)
            else:
                empty_state("Not enough data")
