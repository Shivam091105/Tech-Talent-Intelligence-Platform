"""
Salary Intelligence — salary distributions, comparisons, and analysis.
"""

import streamlit as st
from app.components.metrics_card import page_header, section_header, empty_state
from app.components.chart_helpers import horizontal_bar, histogram, box_plot
...
def render():
    page_header("Salary Intelligence", "Compensation trends across skills, locations, and experience levels")
    ...
    try:
        ...
        with tab2:
            section_header("Top Paying Skills", "Skills with the highest average salaries (min 5 postings)")
            ...
            if not skill_sal_df.empty:
                fig = horizontal_bar(skill_sal_df, x="avg_salary", y="skill_name", color=COLORS["secondary"])
                st.plotly_chart(fig, use_container_width=True)
        ...
        with tab3:
            ...
            if not city_sal_df.empty:
                fig = horizontal_bar(city_sal_df, x="avg_salary", y="city", color=COLORS["accent"])
                st.plotly_chart(fig, use_container_width=True)
            else:
        ...
        with tab4:
            ...
            exp_sal_df = salary_analytics.get_salary_by_experience(country=country, source_dataset=source)
            if not exp_sal_df.empty:
                ...
                wm_sal_df = salary_analytics.get_salary_by_work_mode(country=country, source_dataset=source)
